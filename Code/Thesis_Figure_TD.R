...
##Data Prune
TurkeyDF<-read.csv("../Results/TD/Turkey.csv")
...
DuckDF<-read.csv("../Results/TD/Duck.csv")
...
##Plot
TurkeyCurve<-stat_function(fun=function(x) TurkeyInit[50,4] * exp(TurkeyLam[50,4] * x), geom="line", 
                        aes(colour = "Turkey"),
                        size=.5)
DuckCurve<-stat_function(fun=function(x) DuckInit[50,4] * exp(DuckLam[50,4] * x), geom="line", 
                         aes(colour = "Duck"),
                         size=.5)
BaseDF<-data.frame(
...
TD_Plot<-ggplot(FullDF, aes(x=x,y=y)) + geom_blank() + xlim(0, 1000000) + ylim(0,0.12) +
